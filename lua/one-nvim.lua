...
local function filter_group_style(value)
    return value ~= "background" and value ~= "blend" and value ~= "foreground" and value ~= "special"
end
...
local colorize =
    _USE_HEX and function(command, attributes) -- {{{ †
        command[#command + 1] =
            " guibg=" .. get(attributes.bg, _PALETTE_HEX) .. " guifg=" .. get(attributes.fg, _PALETTE_HEX)
    end or
    _USE_256 and
        function(command, attributes)
            command[#command + 1] =
                " ctermbg=" .. get(attributes.bg, _PALETTE_256) .. " ctermfg=" .. get(attributes.fg, _PALETTE_256)
        end or
    function(command, attributes)
        command[#command + 1] =
            " ctermbg=" .. get(attributes.bg, _PALETTE_ANSI) .. " ctermfg=" .. get(attributes.fg, _PALETTE_ANSI)
    end
...
local function use_background_with(attributes)
    return setmetatable(attributes[go.background], {["__index"] = attributes})
end
